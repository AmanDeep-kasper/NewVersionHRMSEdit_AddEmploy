...
//           <th style={rowHeadStyle(darkMode)}>Absent</th>
//           <th style={rowHeadStyle(darkMode)}>Present</th>
//           <th style={rowHeadStyle(darkMode)}>Week off</th>
//           <th style={rowHeadStyle(darkMode)}>Holiday</th>
//           <th style={rowHeadStyle(darkMode)}>Half&nbsp;Day</th>
//           <th style={rowHeadStyle(darkMode)}>Paid&nbsp;Leave</th>
//           <th style={rowHeadStyle(darkMode)}>Unpaid&nbsp;Leave</th>
...
//             const yearObj = employee.attendance?.find((y) => y.year === filterYear);
//             const monthObj = yearObj?.months?.find((m) => m.month === filterMonth) || {};
//             const dates = monthObj.dates || [];
//             const summary = monthObj.totals || {};
//               console.log(summary.isNcnsSandwich);
...
//   const dateObj = dates.find((x) => x.date === d + 1) || {};
//   const status = dateObj.status || "--";
//   const color =
//     dateObj.color || (darkMode ? "rgba(200,200,200,.4)" : "rgba(0,0,0,.2)");

//   const isHover =
//     selectedAttendance.employeeId === employee.userId &&
//     selectedAttendance.day === d + 1;

//   // Helper derived flags (from backend-provided status)
//   const isHoliday = status === "O" || status === "HD";
//   const isWeekend = status === "W" || status === "WO";
//   const isOnLeave = !!dateObj.isOnLeave;
...
//       style={{
//         whiteSpace: "pre",
//         backgroundColor: isHover ? "#6A5ACD" : color,
//         textAlign: "center",
//         fontWeight: 600,
//         color: "white",
//         verticalAlign: "middle",
...
//       }}
//       onMouseEnter={() =>
//         setSelectedAttendance({ employeeId: employee.userId, day: d + 1 })
//       }
//       onMouseLeave={() =>
//         setSelectedAttendance({ employeeId: null, day: null })
//       }
...
//                       IN&nbsp;<FiLogIn />&nbsp;
//                       <span>
//                         {dateObj.loginTime?.length
//                           ? convertToAmPm(dateObj.loginTime[0])
//                           : "--"}
//                       </span>
//                     </p>
...
//                       OUT&nbsp;<FiLogOut />&nbsp;
//                       <span>
//                         {dateObj.logoutTime?.length
//                           ? convertToAmPm(
//                               dateObj.logoutTime[
//                                 (dateObj.logoutTime?.length || 1) - 1
//                               ]
//                             )
//                           : "Not Logged"}
//                       </span>
...
//                 {/* Net Login / Leave / Holiday details */}
//                 {isOnLeave ? (
//                   <div className="d-flex flex-column gap-1 px-3">
//                     <span>
//                       Duration:&nbsp;
//                       {dateObj.leave?.leaveDuration ||
//                         dateObj?.leaveAttendanceData?.leaveDuration ||
//                         "--"}
//                       {" → "}
//                       {["Casual Leave", "Paid Leave", "Maternity Leave", "Paternity Leave"].includes(
//                         dateObj.leave?.Leavetype ||
...
//                   <div className="d-flex align-items-center gap-1">
//                     <p className="p-1 my-0">
//                       {dateObj.holiday?.holidayName || "Holiday"}
//                     </p>
//                     <p className="p-1 my-0">
//                       {dateObj.holiday?.holidayType || ""}
//                     </p>
...
//                 <td style={rowBodyStyle(darkMode)}>{summary.totalDays || 0}</td>
//                 <td style={rowBodyStyle(darkMode)}>{summary.absent || 0}</td>
//                 <td style={rowBodyStyle(darkMode)}>{summary.present || 0}</td>
//                 <td style={rowBodyStyle(darkMode)} className="text-center">{summary.weekoff || 0}</td>
//                 <td style={rowBodyStyle(darkMode)} className="text-center">{summary.holiday || 0}</td>
//                 <td style={rowBodyStyle(darkMode)} className="text-center">{summary.halfday || 0}</td>
//                 <td style={rowBodyStyle(darkMode)}>{summary.paidLeaves || 0}</td>
//                 <td style={rowBodyStyle(darkMode)}>{summary.unpaidLeaves || 0}</td>
//                 <td style={rowBodyStyle(darkMode)}>{summary.NCNS || 0}</td>
                
//                 <td style={rowBodyStyle(darkMode)}>{summary.Sandwich || 0}</td>
//                 <td style={rowBodyStyle(darkMode)}>{summary.totalPayableDays || 0}</td>
//               {/* --------- Totals Columns ---------------------------------- */}
//               {/* <td style={rowBodyStyle(darkMode)} className="text-center">
//                 {calculateTotal("H", employee) +
//                   calculateTotal("P", employee) +
