...
// import Managerverifytask from "../ManagerCencelledTask";
import ManagerVerifyTask from "../ManagerVerifyTask";
import ManagerNewTask from "../ManagerNewTask";
import ManagerActiveTask from "../ManagerActiveTask";
import ManagerCompletedTask from "../ManagerCompletedTask";
import ManagerRejectTask from "../ManagerRejectedTask";
import ManagerReviewTask from "../ManagerReviewTask";
...
import ManagerCreateTask from "../ManagerCreateTask";
...
const TaskContainer = () => {
  ...
  const renderTaskComponent = () => {
    switch (activeTask) {
      case "activeTask":
        return <ManagerActiveTask />;
      case "taskreview":
        return <ManagerReviewTask />;
      case "taskComplete":
        return <ManagerCompletedTask />;
      case "taskReject":
        return <ManagerRejectTask />;
      case "verifytask":
        return <ManagerVerifyTask />;
      case "createtask":
        return <ManagerCreateTask />;
      default:
        return <ManagerNewTask />;
    }
  };
            ...
            <div className="d-flex flex-wrap gap-3">
              {/* <div className="assigned">
                <button
                  ...
                  style={{
                    background:
                      activeTask === "taskdash" ? "rgba(0, 0, 205, 0.50)" : "#FFFFFF",
                    color: activeTask === "taskdash" ? "#F5F5F5" : "#1b20a4d3",
                    fontWeight: "600",
                  }}
                  onClick={() => setActiveTask("taskdash")}
                >
                 Dashboard
              ...
              <div className="review">
                <button
                  ...
                  style={{
                    background:
                      activeTask === "taskreview" ? "rgba(0, 0, 205, 0.50)" : "#FFFFFF",
                    color:
                      activeTask === "taskreview" ? "#F5F5F5" : "#1b20a4d3",
                    fontWeight: "600",
                  }}
                  onClick={() => {
                    sessionStorage.setItem("task", "taskreview");
                    setActiveTask("taskreview");
                  }}
                >
                  In Review <span class="badge bg-danger"> {
                    tasks.filter(
                      (task) =>
                        task.status === "Pending" &&
                        task.managerEmail.Email === email &&
                        task.employees.some(
                          (taskemp) => taskemp.empTaskStatus === "In Review"
                        )
                    ).length
                  }</span>
                </button>
              ...
              <div className="verify">
                <button
                  ...
                  style={{
                    background:
                      activeTask === "verifytask" ? "rgba(0, 0, 205, 0.50)" : "#FFFFFF",
                    color:
                      activeTask === "verifytask" ? "#F5F5F5" : "#1b20a4d3",
                    fontWeight: "600",
                  }}
                  onClick={() => setActiveTask("verifytask")}
                >
                  Verified Task{" "}
                  <span className="badge bg-danger">
                    {/* {
                    ...
                    } */}
                    {
                      tasks.filter(
                        (task) =>
                          ...
                          task.managerEmail.Email === email &&
                          task.employees.some(
                            (taskemp) => taskemp.empTaskStatus === "Verified"
                          )
                      ).length
                    }
                  </span>
