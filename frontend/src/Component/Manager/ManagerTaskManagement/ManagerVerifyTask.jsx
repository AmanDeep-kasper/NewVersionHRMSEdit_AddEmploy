...
const ManagerVerifyTask = () => {
  ...
  const verifiedTasks = tasks.filter(
    (task) =>
      ...
      task.employees?.some((emp) => emp.empTaskStatus === "Verified")
  );
    ...
    return (
      ...
      >
        ...
        <p className="text-muted mb-0" style={{ maxWidth: "420px" }}>
          There are currently no tasks with verified submissions waiting for
          your action.
        </p>
      </div>
    );
  ...
  const calculateProgress = (task) => {
    ...
    const validEmployees = task.employees.filter(
      (emp) => emp.empTaskStatus !== "Rejected"
    );
    const total = validEmployees.length;
    if (total === 0) return 0;

    const completed = validEmployees.filter((emp) =>
      ["Completed", "Verified"].includes(emp.empTaskStatus)
    ).length;
...
export default ManagerVerifyTask;
