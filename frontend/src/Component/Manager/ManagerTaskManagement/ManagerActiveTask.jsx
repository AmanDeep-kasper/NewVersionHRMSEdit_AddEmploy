...
const ManagerActiveTask = () => {
    ...
    return (
      ...
      >
        ...
        <p className="text-muted mb-0" style={{ maxWidth: "420px" }}>
          There are no pending tasks assigned to you at the moment.
        </p>
      </div>
    );
  }
  ...
  const getPriorityStyle = (priority) => {
    ...
    return (
      styles[priority] || { bg: "#f3f4f6", color: "#4b5563", border: "#d1d5db" }
    );
  };

  const calculateProgress = (task) => {
    const validEmployees =
      task.employees?.filter((emp) => emp.empTaskStatus !== "Rejected") || [];
